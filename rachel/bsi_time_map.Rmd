...
```
```{r}
bsi <- readxl::read_excel(here::here("data", "prepped", "bsi_time_state.xlsx")) %>% 
...
ggsave("facetted_time_boxplot.png")
```
...
```{r}
bsi_time_map <- ggplot(data = bsi_geo_data, 
             aes(x = long, 
                 y = lat,
                 fill = bsi,
                 group = group),
    ...
    theme_map()
animate(bsi_time_map,
        duration = 12) 
anim_save("bsi_time.gif")
```
