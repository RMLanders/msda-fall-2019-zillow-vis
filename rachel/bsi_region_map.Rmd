...
```
```{r}
facetted_region_boxplot <- ggplot(bsi, aes(year, bsi, group = year)) +
  geom_boxplot() + 
  facet_wrap(~region)
...
```{r}
us_boxplot <- ggplot(bsi, aes(year, bsi, group = year)) +
  geom_boxplot()
ggplotly(us_boxplot)
```
```{r}
all_time_region_boxplot <- ggplot(bsi, aes(region, bsi, group = year)) +
  geom_boxplot() +
...
```{r}
current_region_boxplot <- ggplot(bsi %>% subset(date == as.Date('2019-08-01')), aes(region, bsi, group = year)) +
  geom_boxplot()
...
```{r}
bsi_map <- ggplot(data = bsi_geo_data, 
             aes(x = long, 
                 y = lat,
                 fill = days,
                 group = group),
